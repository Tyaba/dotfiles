...
- Codex CLIは`codex mcp-server`でMCPサーバーとして動作し、`mcp.json.erb`で定義。`base-instructions`パラメータでClaude側のルールを動的注入可能。`$CODEX_HOME/AGENTS.md`は直接CLI利用時のグローバルルール。CI環境では利用不可（OAuth認証にブラウザフロー必須）。`config/coding_agents/codex/AGENTS.md.erb`→`$CODEX_HOME/AGENTS.md`でデプロイ
...
- `CODEX_HOME` は codex を起動する経路すべてに届ける必要がある。(1) 対話 CLI → `config/.zsh/lib/apps`（host と共有のファイルなので `DOTFILES_ROLE` ではなく `~/.config/codex/config.toml` の存在で判定）、(2) `~/.mcp.json` の codex エントリ → `mcp.json.erb` の `env`、(3) user scope → `sync-claude-user-mcp.sh` が `env` を `--env` で引き継ぐ。**Claude Code は stdio server をログインシェル経由で起動しないので export だけでは届かない**
