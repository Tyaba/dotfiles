...
Codex MCP サーバー (`mcp__codex__codex`) が利用可能な場合、タスクの性質に基づいて自動的に Codex へ移譲せよ。
...
### 移譲方法
`mcp__codex__codex` ツールで Codex セッションを開始する。プロジェクト固有のコンテキスト（DDD構造、命名規約等）は `base-instructions` パラメータで渡せ。
